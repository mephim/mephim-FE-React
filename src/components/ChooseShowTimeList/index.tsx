...
function ChooseShowTimeList({movie} : IProp) {
  ...
  const [listShowTimeExist, setListShowTimeExist] = useState<any[]>([]);
  ...
  const findShowTime = async (dateDetail: string) => {
    console.log('---date detail: ', dateDetail)
    const showDateId = listShowDateExist?.filter((date) => date.date === dateDetail)[0]?.showDateId || null;
    console.log('---showdateId: ', showDateId)
    if(showDateId === null) {
      setListShowTimeExist([]);
    }
    if (showDateId) {
      const response = await Api.get(`http://localhost:9090/api/movie/find-show-time-by-show-date?movieId=${movie.movieId}&showDateId=${showDateId}`);
        ...
        <div className="day-list">
          {dateList.map((dateWrap: DateWrap, index: number) => (
              <div key={index}>
                ...
                <span className="date-element" onClick={() => findShowTime(dateWrap.dateDetail)}>{dateWrap.date}</span>
              </div>
          ))}
        </div>
      </div>
        ...
        <div className="show-time-wrap">
          ...
          <div className="show-time-detail">
            ...
            {listShowTimeExist.length > 0 && <div className="show-time-list">
              {listShowTimeExist.map((item: any, index) => (
