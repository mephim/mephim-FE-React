...
function Rate() {
    ...
    useEffect(() => {
        findAllRate().then(res => setListRate(res.data.data)).catch(err => console.log(err));
    }, [refresh]);

    const likeThisRate = (rateId: number, isLike: boolean) => {
        reactRate(rateId, !isLike).then(res => {
            setRefresh(!refresh);
        }).catch(err => console.log(err));
    };

    const deleteThisRate = (rateId: number) => {
        deleteRate(rateId).then(res => {
            setRefresh(!refresh);
        }).catch(err => console.log(err));
    };
        ...
        <Table dataSource={listRate}>
            ...
            <Column
                ...
                render={(_: any, record: IRateResponse) => (
                    <Space size='large'>
                        <Button className="d-flex align-items-center justify-content-center" style={{width: 50}} block icon={record.liked ? <LikeFilled /> : <LikeOutlined />}
                                onClick={() => likeThisRate(record.rateId, record.liked)} />
                        <Button className="d-flex align-items-center justify-content-center" style={{width: 50}} block icon={<StopOutlined />}
